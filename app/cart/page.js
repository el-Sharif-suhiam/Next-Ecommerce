...
function page() {
  ...
  React.useEffect(() => {
    ...
    if (cartItems.cartData) {
      const total = cartItems?.cartData?.forEach((ele) => {
        price += ele.attributes.products.data[0].attributes.price;
  ...
  const handleDeleteItem = (id) => {
    ...
    cartApi
      ...
      .then((res) => {
        console.log(res);
        const newData = cartItems?.cartData.filter((ele) => {
          return ele.id !== res.data.data.id;
        });
        cartItems.setCartData(newData);
        console.log("new data = ", newData);
      })
      .catch((error) => console.error("something wrong happend:", error));
  };
          ...
          <div className="mt-8">
                      ...
                      <div className="flex  items-center justify-end gap-2">
                        {/* <form> 
                          ...
                          <input
                            ...
                            value="1"
                            id="Line1Qty"
                            className="h-8 w-12 rounded border-gray-400 border-solid border-[1px] bg-white p-0 text-center text-xs text-gray-800  [-moz-appearance:_textfield] focus:outline-none [&::-webkit-inner-spin-button]:m-0 [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:m-0 [&::-webkit-outer-spin-button]:appearance-none"
                          />
                        </form> */}
            ...
            <div className="mt-8 flex justify-end border-t border-gray-100 pt-10">
              <div className="w-screen max-w-lg space-y-4">
                ...
                <div className="flex justify-end">
                  <button
                    onClick={() => route.push(`/checkout?amount=${priceTotal}`)}
